...
func kernelClauses() string {
	return strings.Join([]string{
		...
		"- 未验证项：本次**没有验证到**的不确定项与未覆盖风险，用固定小节「## 未验证」逐条列出（例如「改了某个分支但没跑对应测试」「依赖了一个未确认的约定」「某条边界没有覆盖」）。写这个小节**不表示停止**——它只把没验证到什么说清楚，是下游判断能否继续自动往下走的直接依据。三节分工是固定的：缺的是**必要前提** → 「## 需要补全」（唯一的停止信号）；采取过的**默认值** → 「## 假设」；**没验证到的东西** → 「## 未验证」。",
