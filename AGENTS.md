...
- `cmd/xhunter/` — CLI **and the assembly layer**: builds the primitive set, the prompt plugins, the context/session/event collaborators and the backend implementations, stitches them into `hunt.Session`, hands the loop `harness.New(provider, session.Prepare, session.OnTurn, session.Finalize)`, maps an SDK value to a protocol factory, and turns configuration into request headers (endpoints are never preset; auth shape comes from config).
