...
| <a id="ia-3-8"></a>IA-3.8 | 已落地 | `TestList_SkipsDefaultExcludedAndReportsThem`、`TestList_DotDirsAreVisible`、`TestList_IncludeReleasesTheExcludedDir`、`TestList_IncludeStarReleasesEverything`、`TestList_VcsDirIsNeverReleased`、`TestList_OrderIsStable`、`TestList_PatternSemantics`、`TestGlob_DirectoryAndDeepPatterns`、`TestGlob_IncludeReachesTheEnumeration`、`TestFind_IncludeReachesTheEnumeration` | — | — |
...
| ~~枚举面的默认排除名单~~（**已落地**，2026-09-25） | `Workspace.List` 不再按"点开头"过滤（`.github/` / `.vscode/` / `.config/` 现在可见——策略并未禁写它们，过滤会造成"能写不能枚举"）；改按**目录名单**默认排除依赖与产物目录（`node_modules` / `vendor` / `dist` / `build` / `target` / `.venv` / `venv` / `__pycache__` / `.tox` / `.gradle` / `.next` / `coverage`），并遵守**排除即上报**（结果里给出被跳过的目录名）与 **`include` 按次放行**（`["*"]` = 全部）；**`.git/` 永不放行**（临时工作树、不进交付、且 hooks 可绕过 INV-10）。契约改为 `List(pattern, include) (ListResult, error)`，`ListResult` 带 `Skipped`。用例 `TestList_SkipsDefaultExcludedAndReportsThem`、`TestList_DotDirsAreVisible`、`TestList_IncludeReleasesTheExcludedDir`、`TestList_IncludeStarReleasesEverything`、`TestList_VcsDirIsNeverReleased` |
