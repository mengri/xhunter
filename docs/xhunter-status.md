...
| **枚举面的默认排除名单** | **已裁决，待落地**（与「不受 gitignore 限制」不是同一条规则）：当前 `Workspace.List` 跳过所有点开头目录（`.github/` / `.vscode/` / `.config/`），只有 `.xhunter` 例外——这条**比 gitignore 更宽**，且造成不对称：策略只禁写 `.xhunter`、并未禁写 `.github`，于是能 `edit .github/workflows/ci.yml` 却 `glob` 不出来（**能写不能枚举**）。全面放开的代价：`find` 走同一套枚举，会在 `node_modules` 上全量检索；解法是配套加**结果限额**，不是缩小可见范围。业界对照（Codex / OpenCode 均默认按 `.gitignore` 排除，但它们**有 shell 可绕过**，缩小的只是默认视图）见设计文档 §6 枚举面 |
