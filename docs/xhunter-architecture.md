...
| 扩展接入 | MCP 客户端、能力描述符与降级、扩展生命周期（FR-13） | `ext.ExtHost`（接口）＋ 装配层注入实现：`ext/syntax`（内置语法级，进程内）或 `ext/mcp`（外挂 stdio 子进程），由部署事实选（实现状态见 xhunter-status.md 状态索引 · H7） |
...
| 动作 | 落点 | 依据 | 说明 |
|---|---|---|---|
| 发起推理 | harness：`Provider.Infer(ctx, Request{Turn, Messages: run.Messages, Tools: run.Tools})` | FR-4.11、FR-9.5 | 请求 = 消息 ＋ 工具声明（**6 个原语 + 1 个控制原语**）；工具声明读自 L1 定格的那一份，与提示词里的工具面说明同源（单一事实源） |
...
| 职责 | 要求 |
|---|---|
| 注册 | **工具面恒定（当前构建 9 + 1）**：`read`/`write`/`edit`/`find`/`glob` ＋ 三个符号原语 ＋ `check`，另加控制原语 `checkpoint`（殿后）。**是否注册由能力接入决定**：尚未实现、且不在本次构建范围内的原语**不注册**（符号三原语随 MS-8 已注册）；**环境能力不决定注册**（扩展不可用、语言未注册时已接入的符号原语仍注册，调用返回结构化错误）。同一构建版本内工具名与参数 schema 不增减，差异只改"调用时会发生什么"（FR-4.11、AC-26） |
...
| 生命周期 | 懒启动（首次调用符号工具时拉起）、随 Hunt 退出回收——回收点在 Hunt 收尾（`Session.Finalize` 末尾，排在 `hunt_end` **之后**：回收本身可能超时，不该把终态一起吞掉）；**崩溃隔离**——扩展死掉不得使主循环静默挂起（INV-3），进程退出 / 握手不上 / 超时 / 输出不是 JSON 一律钉成终态并杀掉进程组；**注册即副作用、卸载即撤销**：扩展注册的工具 schema / 符号能力 / 能力描述符随其卸载全部撤销 |
| 通道与不可信边界 | 仅本地 stdio 子进程；传输与握手走 MCP 标准，符号能力方法走自有命名空间 `xhunter/*`（**不进 `tools/*`**，因此与「扩展不新增工具名」不冲突；不碰 `sampling`）。子进程**不继承**当前环境——只拿装配层显式点名授予的那几项（`XHUNTER_*` 里的凭据不会漏出去）；`ExtHost` 接口本身没有任何写方法（类型级保证） |
| 降级 | 扩展缺失/启动失败/超时 → **符号原语**不可用：调用返回结构化错误并显式提示"改用文本原语"，基础原语不受影响，任务继续（FR-13.4、AC-12、AC-26）。符号原语**已随 MS-8 注册**；**注册之后**扩展缺失 / 启动失败 / 超时只改调用结果、**不改工具面**（不撤回） |
...
门禁**不是新工具**，而是 `check` 原语的具名条目——原语面是 9 个（5 基础 ＋ 3 符号 ＋ 1 门禁，另有控制原语 `checkpoint`；**这 9 个是设计面**：本次构建实际注册的工具面以 §7.3「注册」行为准；符号三原语随 MS-8 已进入注册范围，当前为 9 + 1），门禁不占新的工具名。

**清单来源（按优先级，FR-5.2b）**：① **Bounty 下发**（平台显式配置，可覆盖或禁用仓库声明）② **仓库声明**：仓库根的 `gates.yml`（不是 `.xhunter/` 下的引擎私有文件——别的工具也可以读它）③ 无 → 不设门禁。（档位取值 `bounty` / `repo` / `none`，由 `hunt.GateSource` 承担——来源契约与结果文件 `gates` 形状已在 D2 定义。）
...
**例外（**已于 2026-09-25 作废**，保留只为沿革）：装配扩展点可以先定义后实现（未冻结期口径）**。上面的规则管的是**能力声明**（面向供应商/后端，如 `llm.Caps`、`ext.ExtCaps`）——那类仍然"只声明用到的、不预埋行为类字段"。但**装配扩展点**（装配层能塞东西的空位，如 `hunt.Policy` 的止损入口 `ObserveFailure` / `DeniedCount`）不同：它**可以在实现之前先定义并装配**，**未实现时以 `panic` 哨兵装配**——走到即炸、绝不静默。**本例外已作废**：四处 panic 哨兵已全部回收（策略两处随 MS-3、材料 `Load` 随 MS-7、`ext/ext.go` 四处随 MS-8），代码里已无"走到即炸"。**新代码不再适用它**——装配扩展点要么已实现、要么**显式失败**或**如实降级**。实现状态见 xhunter-status.md §5。
...
| IA-3.8 | 点开头目录一律可见；依赖与产物目录按**默认名单**跳过并**上报目录名**（`ListResult.Skipped`），`include`（`*` = 全部）按次放行；**`.git/` 永不放行**；顺序稳定；**模式语义**：不含 `/` 按文件名（任意深度）、含 `/` 按路径、`**` 表示零到多层、空模式是显式错误 | `TestList_SkipsDefaultExcludedAndReportsThem`、`TestList_DotDirsAreVisible`、`TestList_IncludeReleasesTheExcludedDir`、`TestList_IncludeStarReleasesEverything`、`TestList_VcsDirIsNeverReleased`、`TestList_OrderIsStable`、`TestList_PatternSemantics`（osfs）、`TestGlob_DirectoryAndDeepPatterns`、`TestGlob_IncludeReachesTheEnumeration`、`TestFind_IncludeReachesTheEnumeration`（basic） |
...
| IA-7.5 | `CanResolve == false` 时重命名**仍然注册**（随 MS-8 接入后；工具名与参数 schema 不变），调用返回结构化错误且**不降级为文本路径** | （实现状态见 xhunter-status.md 状态索引 · IA-7.5） |
...
| IA-11.6 | `Diff` 产出改动文件清单、`Patch` 产出**可应用到基线的统一 diff**（FR-6.1、AC-1）；失败不阻断收尾（`Finalize` 只记 warn）。排除会话材料目录 `.xhunter/<session_id>/**`（`RepoRef.MaterialDir`），**同目录下其他路径不排除**（如 `skills.draft/**` 属交付内容、要进 diff 供人 review） | `TestDiff_ListsFilesChangedSinceBaseline`、`TestPatch_AppliesCleanlyToBaseline`（`git apply` 验证）、`TestDiff_ExcludesMaterialDirButKeepsSkillsDraft`、`TestDiff_NoMaterialDirKeepsEverything`、`TestEndToEnd_LocalRunProducesDeliveryCommit`（deliverable 事件 ＋ 补丁应用到基线） |
