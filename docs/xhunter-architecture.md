...
| 职责 | 要求 |
|---|---|
| 注册 | **工具面恒定（同一构建内 6 + 1）**：一期 `read`/`write`/`edit`/`find`/`glob`/`check` ＋ 控制原语 `checkpoint`（殿后）。**是否注册由能力接入决定**：尚未实现、且不在本次构建范围内的原语**不注册**（符号三原语随 MS-8 接入时注册）；**环境能力不决定注册**（扩展不可用、语言未注册时已接入的符号原语仍注册，调用返回结构化错误）。同一构建版本内工具名与参数 schema 不增减，差异只改"调用时会发生什么"（FR-4.11、AC-26） |
...
门禁**不是新工具**，而是 `check` 原语的具名条目——原语面是 9 个（5 基础 ＋ 3 符号 ＋ 1 门禁，另有控制原语 `checkpoint`；**这 9 个是设计面**：本次构建实际注册的工具面以 §7.3「注册」行为准，一期为 6 + 1，符号三原语随 MS-8 接入后进入注册范围），门禁不占新的工具名。
