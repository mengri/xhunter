...
| `XHUNTER_EXT_COMMAND` | — | **外挂符号后端**的可执行文件（argv[0]）。**不配**即用内置语法级后端（进程内，不需要额外安装任何东西）；配了就走 MCP over stdio 子进程。后端起不来时符号原语返回结构化错误（模型退回文本寻址）、**任务不失败**，也不静默退回内置——静默退回会让精度档位悄悄变化而不上报 |
| `XHUNTER_EXT_ARGS` | — | 外挂后端的入参，**按空白切分**（不支持引号转义） |
| `XHUNTER_EXT_TIMEOUT` | — | 单次调用的上界（正 Go duration，缺省 30s）。超时即**隔离**该扩展（杀进程、钉成终态），绝不让主循环挂住；**取值非法（含 `0`、负数、非 duration）即启动期退出 1** |
| `XHUNTER_EXT_LANGUAGES` | — | 后端覆盖的语言清单（逗号分隔，如 `go,rust`）：进能力指纹（`effective_config.ext`）。指纹在装配冻结时就读，那时还没问过后端，语言只能由部署侧声明。**有空项即启动期退出 1** |
| `XHUNTER_EXT_ENV` | — | **点名授予**的环境变量名（逗号分隔，如 `PATH,HOME`）：外挂进程**默认一个环境变量都不继承**（`XHUNTER_*` 里可能有凭据），只拿这里点名、且当前环境里确实存在的那几项。**点名了不存在的名字即启动期退出 1** |

**前缀即分组**（命名是刻意的，防错靠名字而不是靠文档提醒）：`XHUNTER_MODEL_*` 是**模型接入事实**（模型是什么——上限两项必填、其余可选）；`XHUNTER_BUDGET_*` 是**任务预算**（三项都可选，不配即不限）；`XHUNTER_REPO_*` 是仓库事实；`XHUNTER_EXT_*` 是**符号后端事实**（整组可选，不配即内置后端）。几组的**必填性相反**，因此让名字完全不重叠。
